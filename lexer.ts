...
import { isNumber, isValidIdentifier, isBooleanSymbol } from "./utils";
export class Lexer {
  ...
  getNextCharacter() {
    if (!this.input || this.input.length < this.afterPosition)
      throw new Error("getNextCharacter is out of bounds.");
    return this.input[this.afterPosition];
  }
  getCharacter() {
    if (!this.input || this.input.length < this.position)
      throw new Error("Called getCharacter out of bounds");
  ...
  getToken() {
    ...
    if (this.getCharacter() == '"' || this.getCharacter() == "'")
      return tokenizeString.bind(this)();
  ...
  getTypeFromCharValue(char: string | undefined) {
    const keyValue = Object.entries(tokens).filter((kv) => {
      return kv[1] == char;
    });
