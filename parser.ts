...
import {
  ...
  parseStatementBlock,
} from "./parsers/statementParsers";
const expressionEndTokens = [tokens.COMMA, tokens.SEMICOLON, tokens.RPAREN];
export class Parser {
  ...
  infixParsers: Map<
    string,
    (left: Expression, precedance: number) => InfixExpression
  >;
  constructor(tokenList?: Token[]) {
    ...
    this.addPrefix("BOOL_TRUE", BooleanLiteralParser.bind(this, true));
    this.addPrefix("BOOL_FALSE", BooleanLiteralParser.bind(this, false));
    ...
    this.addInfix(tokens.LOG_OR, GenericInfixParser.bind(this));
    this.addInfix(tokens.LOG_AND, GenericInfixParser.bind(this));
  ...
  addInfix(
    token: string,
    parser: (left: Expression, precedance: number) => InfixExpression
  ) {
    this.infixParsers.set(token, parser);
  }
  getPrefixParser(token: Token) {
    if (token.type == undefined)
      throw new Error("Undefined token type in getPrefixParser");
    return this.prefixParsers.get(token.type);
  }
  getInfixParser(token: Token) {
    if (token.type == undefined)
      throw new Error("Undefined token type in getInfixParser");
  ...
  getAfterToken(): Token | undefined {
    if (!this.tokenList)
      throw new Error("Accessing after token from token list out of bounds.");
  ...
  parseExpression(precedance: number = Precedance.LOWEST) {
    ...
    while (
      this.getToken() &&
      !expressionEndTokens.includes(this.getToken()!.value) &&
      this.getPrecedance(this.getToken()!) > precedance
    ) {
  ...
  parsePrefix() {
    ...
    if (!parser)
      throw new Error(
        "Prefix parser for this token is undefined " + token!.type
      );
    return parser();
  }
  parseInfix(left: Expression, precedance: number) {
    ...
    if (!parser)
      throw new Error(
        "Infix parser for this token does not exist, " + this.getToken()!.type
      );
    return parser(left, precedance);
  }
  parseStatement() {
    if (
      this.getToken()?.type == tokens.IDENTIFIER &&
      this.getAfterToken()?.value !== tokens.LPAREN
    ) {
      return parseAssignmentStatement.bind(this)();
    }
    ...
    switch (this.getToken()!.value) {
      ...
      default:
        try {
          ...
          throw new Error(
            "Dont know how to parse this statement: " +
              `'${this.getToken()!.type}'`
          );
        }
    }
  ...
  getPrecedance(token: Token) {
    if (token.type == undefined)
      throw new Error("Cannot get precedance for undefined token type");
    return Precedance[token.type] ?? 1;
  }
  getAfterPrecedance() {
    const token = this.getAfterToken();
    if (!token)
      throw new Error("next token in getAfterPrecedance is undefined");
